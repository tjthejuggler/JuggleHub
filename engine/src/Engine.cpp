            ...
            switch (command.type()) {
                ...
                case juggler::v1::CommandRequest::CALIBRATE_COLOR:
                    if (dnn_tracker_ && !last_color_frame_.empty()) {
                        ...
                        // Check if this is a ball registry color sample (color_name is ball_id)
                        auto& registry = dnn_tracker_->getBallRegistry();
                        if (registry.getBall(command.color_name()) != nullptr) {
                            // This is a ball registry color sample
                            cv::Mat hsv_frame;
                            cv::cvtColor(last_color_frame_, hsv_frame, cv::COLOR_BGR2HSV);
                            
                            std::string lighting = "unknown";
                            if (command.module_args().count("lighting")) {
                                lighting = command.module_args().at("lighting");
                            }
                            
                            bool success = registry.addColorSample(command.color_name(), hsv_frame, click_point, lighting);
                            if (success) {
                                response.set_message("Color sample added to ball: " + command.color_name());
                            } else {
                                response.set_success(false);
                                response.set_message("Failed to add color sample to ball: " + command.color_name());
                            }
                        } else {
                            // Legacy color tracker calibration
                ...
                case juggler::v1::CommandRequest::DISABLE_FEATURE: {
                    // Handle ball management commands
                    const std::string& feature = command.feature_name();
                    DEBUG_LOG("Processing feature command: ", feature);
                    ...
                    if (feature == "get_all_balls") {
                        // Return all registered balls as JSON
                        auto all_balls = registry.getAllBalls();
                        json balls_json = json::array();
                        for (const auto* ball : all_balls) {
                            balls_json.push_back(ball->toJson());
                        }
                        response.set_message(balls_json.dump());
                        
                    } else if (feature == "get_active_balls") {
                        // Return active balls as JSON
                        auto active_balls = registry.getActiveBalls();
                        json balls_json = json::array();
                        for (const auto* ball : active_balls) {
                            balls_json.push_back(ball->toJson());
                        }
                        response.set_message(balls_json.dump());
                        
                    } else if (feature == "create_ball") {
                        // Create new ball
                        std::string display_name = "New Ball";
                        if (command.module_args().count("display_name")) {
                            display_name = command.module_args().at("display_name");
                        }
                        std::string ball_id = registry.createBall(display_name);
                        response.set_message("Ball created with ID: " + ball_id);
                        
                    } else if (feature == "delete_ball") {
                        // Delete ball
                        if (!command.module_args().count("ball_id")) {
                            response.set_success(false);
                            response.set_message("Missing ball_id parameter");
                            break;
                        }
                        std::string ball_id = command.module_args().at("ball_id");
                        bool success = registry.deleteBall(ball_id);
                        if (!success) {
                            response.set_success(false);
                            response.set_message("Failed to delete ball: " + ball_id);
                        } else {
                            response.set_message("Ball deleted: " + ball_id);
                        }
                        
                    } else if (feature == "activate_ball") {
                        // Activate ball
                        if (!command.module_args().count("ball_id")) {
                            response.set_success(false);
                            response.set_message("Missing ball_id parameter");
                        ...
                        std::string ball_id = command.module_args().at("ball_id");
                        bool success = registry.activateBall(ball_id);
                        if (!success) {
                            response.set_success(false);
                            response.set_message("Failed to activate ball: " + ball_id);
                        } else {
                            response.set_message("Ball activated: " + ball_id);
                        }
                        
                    } else if (feature == "deactivate_ball") {
                        // Deactivate ball
                        if (!command.module_args().count("ball_id")) {
                            response.set_success(false);
                            response.set_message("Missing ball_id parameter");
                        ...
                        std::string ball_id = command.module_args().at("ball_id");
                        bool success = registry.deactivateBall(ball_id);
                        if (!success) {
                            response.set_success(false);
                            response.set_message("Failed to deactivate ball: " + ball_id);
                        } else {
                            response.set_message("Ball deactivated: " + ball_id);
                        }
                        
                    } else if (feature == "remove_color_sample") {
                        // Remove color sample
                        if (!command.module_args().count("ball_id") || !command.module_args().count("sample_index")) {
                            response.set_success(false);
                            response.set_message("Missing ball_id or sample_index parameter");
                            break;
                        }
                        std::string ball_id = command.module_args().at("ball_id");
                        int sample_index = std::stoi(command.module_args().at("sample_index"));
                        bool success = registry.removeColorSample(ball_id, sample_index);
                        if (!success) {
                            response.set_success(false);
                            response.set_message("Failed to remove color sample");
                        } else {
                            response.set_message("Color sample removed");
                        }
                        
                    } else if (feature == "new_tracking_system") {
                        // Enable new tracking system
                        dnn_tracker_->setUseNewBallTracking(command.type() == juggler::v1::CommandRequest::ENABLE_FEATURE);
                        response.set_message(command.type() == juggler::v1::CommandRequest::ENABLE_FEATURE ? "new" : "legacy");
                        
                    } else if (feature == "get_tracking_mode") {
                        // Get tracking mode
                        bool using_new = dnn_tracker_->isUsingNewBallTracking();
                        response.set_message(using_new ? "new" : "legacy");
                        
                    } else if (feature == "save_ball_registry") {
                        // Save ball registry
                        std::string filepath = "ball_registry.json";
                        if (command.module_args().count("filepath")) {
                            filepath = command.module_args().at("filepath");
                        }
                        bool success = registry.saveToFile(filepath);
                        if (!success) {
                            response.set_success(false);
                            response.set_message("Failed to save ball registry");
                        } else {
                            response.set_message("Ball registry saved to: " + filepath);
                        }
                        
                    } else if (feature == "load_ball_registry") {
                        // Load ball registry
                        std::string filepath = "ball_registry.json";
                        if (command.module_args().count("filepath")) {
                            filepath = command.module_args().at("filepath");
                        }
                        bool success = registry.loadFromFile(filepath);
                        if (!success) {
                            response.set_success(false);
                            response.set_message("Failed to load ball registry");
                        } else {
                            response.set_message("Ball registry loaded from: " + filepath);
                        }
                        
                    } else {
                        response.set_success(false);
                        response.set_message("Unknown feature: " + feature);
                    }
                    break;
                }
                default:
