...
def process_directory(v5_dir):
    v6_3_dir = v5_dir.replace("V5", "V6.3")
    if not os.path.exists(v6_3_dir):
        shutil.copytree(v5_dir, v6_3_dir)

    for root, _, files in os.walk(v6_3_dir):
        for file in files:
    ...
    for image_id in data.get('_via_img_metadata', {}):
        ...
        for region in image_data.get('regions', []):
            ...
            if shape_attributes.get('name') == 'rect':
                ...
                # Grow by 2 pixels
                shape_attributes['x'] = x - 2
                shape_attributes['y'] = y - 2
                shape_attributes['width'] = width + 4
                shape_attributes['height'] = height + 4
