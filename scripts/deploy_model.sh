...
# deploy_model.sh - Automate the deployment of fine-tuned YOLO models
# Usage: ./deploy_model.sh <model_name> [options]
...
if [[ "$MODEL_SIZE" == "nano" ]]; then
    TARGET_XML="yolo11n.xml"
    TARGET_BIN="yolo11n.bin"
else
    TARGET_XML="yolo11s.xml"
    TARGET_BIN="yolo11s.bin"
fi
